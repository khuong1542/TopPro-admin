function JS_Blog(baseUrl, module, action) {
    $("#main_blogs").attr('class', 'nav-link active');
    this.baseUrl = baseUrl;
    this.module = module;
    this.action = action;
    this.urlPath = baseUrl + '/' + module + (action != '' && action != undefined ? '/' + action : '');
    this.oFormIndex = '#frmBlogs_index';
    this.oFormAdd = '#frmBlogs_add';
    this.currentPage = 1
    this.perPage = 15;
}
/**
 ...
 * Danh sách
 */
JS_Blog.prototype.loadList = function (currentPage = 1, perPage = 15) {
    var myClass = this;
 ...
 * Form thêm mới
 */
JS_Blog.prototype.create = function () {
    var myClass = this;
 ...
 * Form sửa
 */
JS_Blog.prototype.edit = function (id) {
    var myClass = this;
    ...
    if (listId == '') {
        Library.alertMessage('warning', 'Cảnh báo', 'Chọn một bài viết để sửa!');
        return false;
    }
    if (listId > 1) {
        Library.alertMessage('warning', 'Cảnh báo', 'Chọn một bài viết để sửa!');
 ...
 * Lưu thông tin
 */
JS_Blog.prototype.update = function (type) {
    var myClass = this;
 ...
 * Xóa thông tin
 */
JS_Blog.prototype.delete = function () {
    var myClass = this;
    var listId = '';
    var chk_item_id = $('#table-data').find('input[name="chk_item_id"]');
    $(chk_item_id).each(function () {
    ...
    if (listId == '') {
        Library.alertMessage('warning', 'Cảnh báo', 'Chọn ít nhất một bài viết để xoá!');
        return false;
    }
